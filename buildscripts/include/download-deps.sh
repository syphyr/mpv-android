...
if [ ! -d mbedtls ]; then
	...
	if [ ! -f mbedtls-$v_mbedtls.tar.gz ]; then
		$WGET -q --show-progress https://github.com/ARMmbed/mbedtls/archive/mbedtls-$v_mbedtls.tar.gz
	fi
...
if [ ! -d openssl ]; then
	...
	if [ ! -f openssl-$v_openssl.tar.gz ]; then
		$WGET -q --show-progress https://www.openssl.org/source/openssl-$v_openssl.tar.gz
	fi
